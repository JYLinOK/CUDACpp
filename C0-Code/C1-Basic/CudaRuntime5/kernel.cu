...
int main() {
    ...
    auto start_all = system_clock::now();  // 获取当前时间-1
    ...
    for (int N = 1; N <= 1024; N++) {
        ...
        cout << "本次调用核函数时间：" << double(duration.count()) * microseconds::period::num / microseconds::period::den << "秒" << endl << endl;
        // ===========================================================
    }

    auto end_all = system_clock::now();  // 获取当前时间-2
    auto duration = duration_cast<microseconds>(end_all - start_all);  // 需要在函数内部使用duration_cast
    cout.setf(ios::fixed, ios::floatfield); // 设置使用十进制计数法，不使用科学计数法
    cout << "总共运行核函数时间：" << double(duration.count()) * microseconds::period::num / microseconds::period::den << "秒" << endl << endl;
